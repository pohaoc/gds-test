...
static void *thread_fn(void *data)
{
	...
	ret = cuFileRead(t->cfr_handle, t->devPtr, t->sz / NUM_THREADS, t->offset, t->devPtr_offset);
	if (ret < 0) {
		fprintf(stderr, "cuFileRead failed with ret=%d\n", ret);
	}


	fprintf(stdout, "Read Success file-offset %ld readSize %ld to GPU 0 buffer offset %ld size %ld Bytes\n",
			(unsigned long) t->offset, t->sz / NUM_THREADS, (unsigned long) t->offset, t->sz / NUM_THREADS);
...
int main(int argc, char **argv) {
	...
	cout << "Time with " << NUM_THREADS << " threads: " << (float) duration_cast<milliseconds>(high_resolution_clock::now() - thread_start).count() << "msec\n";
